...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($newPassword == $confirmPassword) {
        try {
            ...
            $checkPasswordSql = "SELECT * FROM institution WHERE CodeGresa = ? AND MotDePasse = ?";
            $checkPasswordStmt = $conn->prepare($checkPasswordSql);
            $checkPasswordStmt->bindParam(1, $_SESSION["user_id"]);
            $checkPasswordStmt->bindParam(2, $currentPassword);
            $checkPasswordStmt->execute();

            if ($checkPasswordStmt->rowCount() > 0) {
                // Update password in the database without hashing
                $updatePasswordSql = "UPDATE institution SET MotDePasse = ? WHERE CodeGresa = ?";
                $updatePasswordStmt = $conn->prepare($updatePasswordSql);
                $updatePasswordStmt->bindParam(1, $newPassword);  // Do not hash the password
                $updatePasswordStmt->bindParam(2, $_SESSION["user_id"]);
                $updatePasswordStmt->execute();

                // Optionally, update the session variable
                $_SESSION["user_password"] = $newPassword;

                // Set the success message
                $successMessage = "تم تغيير كلمة المرور بنجاح";

                // Rediriger vers la page de succès si la modification du mot de passe est réussie
